...
def split_text(text: str, split_by: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    for line in text.splitlines():
        ...
        elif split_by == "period":
            # Split by period and keep the period in the parts
            parts.extend([sentence + '.' for sentence in line.split('.') if sentence])
        else:
...
def _get_response_logit_probs(dataset, model, tokenizer, response_length, batch_size):
    ...
    for batch in loader:
        ...
        with ch.no_grad(), ch.amp.autocast(device_type=str(model.device)):
            output = model(**batch)
