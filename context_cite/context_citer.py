...
class ContextCiter:
    ...
    def _compute_masks_and_logit_probs(self) -> None:
        ...
        for start in range(0, total, self.batch_size):
            ...
            torch.cuda.empty_cache()  # Free up GPU memory after processing each batch.
    ...
    def get_attributions(
        ...
        if not self._cache.get("filtered_partition"):
            ...
            # Truncate filtered_ctx if it is too long.
            filtered_tokens = self.tokenizer(filtered_ctx, add_special_tokens=False)["input_ids"]
            max_model_length = self.tokenizer.model_max_length
            if len(filtered_tokens) > max_model_length:
                filtered_ctx = self.tokenizer.decode(filtered_tokens[:max_model_length])
                print(f"Filtered context truncated from {len(filtered_tokens)} to {max_model_length} tokens.")
            partitioner = SentencePeriodPartitioner(filtered_ctx)
            partitioner.split_context()
            self._cache["filtered_partition"] = partitioner
